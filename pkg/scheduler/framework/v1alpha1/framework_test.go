...
func (ru *TestReserveUnreservePlugin) Reserve(pc *PluginContext, p *v1.Pod, nodeName string) *Status {
	return nil
}

func (ru *TestReserveUnreservePlugin) Unreserve(pc *PluginContext, p *v1.Pod, nodeName string) {
	ru.unreserveCalled++
}
...
func (ru *TestReserveFailedUnreservePlugin) Reserve(pc *PluginContext, p *v1.Pod, nodeName string) *Status {
	return NewStatus(Error, "Always failed")
}

func (ru *TestReserveFailedUnreservePlugin) Unreserve(pc *PluginContext, p *v1.Pod, nodeName string) {
	ru.unreserveCalled++
}
...
func TestReserveAndUnreservePlugins(t *testing.T) {
	...
	t.Run("TestReserveAndUnreservePlugins", func(t *testing.T) {
		...
		pod := &v1.Pod{}
		reservedPlugins, status := f.RunReservePlugins(nil, pod, "")
		if status.IsSuccess() {
			t.Errorf("status must be error")
		}
		f.RunUnreservePlugins(nil, pod, "", reservedPlugins)
